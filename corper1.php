...
error_reporting(E_ERROR||E_WARNING);
	$ty=$_REQUEST['scode'];
	$op=mysqli_query($conn,"SELECT DISTINCT type FROM taraba_corper");
	
		
	
?>

<form action="corper1.php" method="post" name="myForm" role="form">
<div class="form-group">
<label for="type">
   	Select Type:
</label>
...
$ty=$_REQUEST['scode'];
if($ty==true){
$c=mysqli_query($conn,"SELECT COUNT(*) FROM taraba_corper WHERE type='$ty'");
	while ($cp=mysqli_fetch_array($c)) {
echo "<b>Total: </b><b style='color:#0B610B;'>".$cp['COUNT(*)']."</b>";
}
if($ty!='Select One'){
$query=mysqli_query($conn,"SELECT * FROM taraba_corper WHERE scode ='$ty'");
